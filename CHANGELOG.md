...
### Added
- **The guide says when a tire's own retailer is out of stock, and points at the other retailer when it has the tire.** The candidates' stock wording (2.9.0) now reaches the guide tire. A new `RTG_Stock_Sync` runs last in the nightly catalog run, after the link and price syncs, and projects it onto each tire: `decide()` (pure) reads the listings matched to the tire that were seen in the last three days (`FRESH_DAYS`, the link sync's window) and carry stock wording; the retailer the link leads to (`RTG_Price_Sync::resolve_link_retailer()`) decides, one sellable listing outranking a sold-out one; when it is out of stock, the cheapest fresh, in-stock listing at the other retailer on an affiliate-tracked link (`RTG_Link_Sync::classify()`) is kept as the alternative. Only the linked retailer's word ever counts, for the same reason only its price does. Four tire columns hold it (`stock_status`, `stock_checked_at`, `stock_alt_retailer`, `stock_alt_link`; migration 30, `DB_VERSION` 30), written through `RTG_Database::update_stock_data()`, a whitelist writer modeled on the Roamer sync's that holds `updated_at` where it was, so a nightly check never makes a hand-typed price look freshly reviewed. Tires with no verdict are not written. `note()` is what a page says: only a fresh out-of-stock verdict earns a line ("Out of stock at Tire Rack", with the check date in its title); in stock is the normal state and says nothing, and a verdict older than the window is withheld rather than shown stale. Run results are kept in `rtg_stock_sync_results`.
  - **Cards.** `to_frontend_row()` carries the four fields at indexes 30 to 33 (`validation.js` exempts 33 from the entity strip like the other URLs; `safeLinkURL()` validates it at use), `stockNote()` in `pricing.js` mirrors `note()` and is fed `stockFreshDays` from the settings, and the card puts the amber line where the "as of" price note goes, taking that line while it applies. Under the button, a green "In stock at SimpleTire" link (`.tire-card-alt-link`) offers the other retailer; the click delegation in `rivian-tires.js` counts it as a purchase click. The stale "index 31" comment on the retailer label reads 29 now.
  - **Tire page.** The Average price tile's note carries the same line, and a green secondary CTA (`.rtg-tp-cta-alt`) offers the other retailer, tracked as a purchase click by `tire-page.js`. The Offer's schema.org availability, hardcoded InStock since the schema shipped, is OutOfStock while the note shows.
  - **Admin.** The tire edit form's Price field gains a read-only line with what the nightly run last learned: where the price was synced from and when, and the retailer's stock with its check date and the alternative. The save handler leaves these columns alone.

### Tests
- `tests/test-stock-sync.php` (new): a fresh in-stock listing is in stock, an out-of-stock one is out; a sellable listing outranks a sold-out one; the other retailer's in-stock tracked listing is offered cheapest first; an alternative that is sold out, untracked or stale is not; stock at the other retailer alone decides nothing; stale or silent listings and untracked links give no verdict; the note names the retailer and the day, and says nothing for in stock or a stale verdict; the whitelist writer leaves `updated_at` untouched and writes NULL. `tests/test-database.php`: every row producer emits 34 columns with the stock fields at 30 to 33. `tests/test-activator.php`: the tires table has the columns, and migration 30 adds them once to a table without them. `tests/test-pricing.mjs`: `stockNote()` shows a fresh verdict with the retailer and day, withholds in-stock and stale ones, and falls back on a bad window. `npm test`, `php -l` and the contract checks pass locally; the PHPUnit suite runs in CI.
...
### Changed
- **One dialog for the whole guide, and one rule for when to use it.** Anything with its own URL or a form worth keeping is a page (the tire page, compare, writing a review, the Changelog page, a member's reviews); a short interruption that hands you back to where you were is a dialog (the phone filter sheet, Help me choose, the Changelog quick view, the (i) info notes, the photo lightbox). Before, five dialogs were built five ways: three shared the review modal's shell but flipped their close button to the left on phones, the filter sheet had its own header and z-index, the two (i) note dialogs were inline styles with no phone layout (the tire page's had no close button), and the lightbox was a white box with none either.
  - The shell is `frontend/css/rtg-dialog.css` and `frontend/js/modules/dialog.js` (`openDialog()`, `dialogButton()`), on the guide page and, as `frontend/js/rtg-dialog.js` bundled to `window.RTG_DIALOG`, on the tire page; `esbuild.config.mjs` builds both, `RTG_Frontend` and `RTG_Tire_Page` enqueue them (the stylesheet ahead of `rivian-tires.css`, so the guide's rules win a tie). Desktop: a centered card (`--rtg-dialog-width`, 520px; `--sm` 400, `--lg` 600, `--xl` 800), 16px radius, fade plus a 20px settle. Phone (640px and under): a bottom sheet rounded at the top, sized to its content up to 92dvh, sliding up on the sheet curve, with `env(safe-area-inset-bottom)` under the footer. Everywhere: title left, a bordered 40px close button with a drawn X at right, a scrolling body between a fixed header and footer, `z-index: 100000`, one `body.rtg-dialog-open` scroll lock (counted, so nested dialogs release it once), Escape, a Tab trap that skips hidden controls, and focus returned to the opener. The tire page passes the admin's color overrides to the shell under the guide's token names.
  - `advisor.js`, `whats-new.js`, `tooltips.js`, `image-modal.js` and `tire-page.js` build on it; every `.rtg-review-modal-*` class, the inline `style.cssText` dialogs and their hover handlers, the `#imageModal` markup and the dead `.tooltip-container` / `.floating-tooltip` rules are gone. The filter sheet's header, body and footer carry the shell's classes in `frontend/templates/tire-guide.php`, so it matches the other sheets to the pixel (16px gutters, the same close button, Clear all beside a full-width Show N tires) and shares the z-index and lock; its own rules in `rivian-tires.css` shrink to the panel and the row-mode `display: none` / `display: contents`. Start over sits in the shell's header as a small secondary button.
- **Writing a review is the review page, only.** The in-guide review modal (`openReviewModal()` in `ratings.js`, its CSS and the guide's toast) was a strict subset of `/tire-review/` (no detail ratings, vehicle, miles, owner flag, tire search, duplicate check or success screen) and its one remaining trigger was a star tapped on a card; every visible Write a review button already went to the page. A star tap now opens the review page with the tire and that star set: `rtgData.settings.tireReviewUrl` is localized, `RTG_Tire_Review` reads `?rating=1..5` into `rtgTireReview.preselectedRating`, and `tire-review.js` applies it after the tire (Change tire clears both parameters). `ratings.js` keeps loading, drawing and posting ratings. README and the module docblocks say so.

### Tests
- `npm test`, `php -l` and the contract checks pass. The shell was exercised in headless Chromium at 390px and 1280px: an info note (header, footer and close button in place, Got it focused, body scroll locked, Escape closes and releases the lock), a tall dialog (header and footer fixed, body scrolling), the lightbox (photo centered, close button top-right), the filter sheet (same header and footer as the other dialogs, last section ending at the footer, z-index 100000) and the desktop chip row (header and footer absent, popover under its pill).
...
### Changed
- **The guide's filter panel is a chip bar.** The old panel spent two rows on chrome before any control (a "Filter, Sort, and Compare" title, then a Search button that repeated what typing already did), showed no summary of what was on, and gave the vehicle choice the same weight as a dropdown. The new `frontend/templates/tire-guide.php` panel is: the search field (magnifier inside, `type="search"`, no button) beside Help me choose; the vehicle switch, a divider, then one pill per filter (Size, Brand, Category, Price, Warranty, 3PMS, OEM); and a results line with the count, a filter tally, Clear all, Write a review and a labelled Sort. A pill opens a popover under it: the three list pills mirror their select's current options as a list of buttons (optgroups become headings, the "(12)" count a muted figure, disabled options stay disabled), built at open time so vehicle cascades and per-option counts hold; Price and Warranty carry the slider with a value readout, presets (Under $250/$350/$450/Any; Any/40k+/50k+/60k+/70k+) and Reset/Done; 3PMS and OEM are press-to-toggle pills with a check disc when on. A set pill shows its value in accent, so the row is its own summary, and the old chips region under the panel is gone with the "N tires match your filters" banner. Escape or a click outside closes a popover; a popover that would run off the right edge flips to the pill's right edge.
  - The real controls did not move: `#filterSize`, `#filterBrand`, `#filterCategory`, `#priceMax`, `#warrantyMin`, `#filter3pms`, `#filterOEM`, `#sortBy`, `#searchInput` and `#vehicleToggle` keep their ids and sit inside the popovers, and the new module `frontend/js/modules/filter-bar.js` drives them through their own `input` events. Everything that reads them (URL state, analytics, server-side mode, the advisor, shortcode prefilters, the no-results suggestions) is unchanged. `syncFilterBar()` replaces `renderActiveFilterChips()`, `updateFilterResultCount()` and `updateMobileFilterBadge()` in the filter pass and in `server.js`; the count reads "229 tires" rather than "Showing 229 tires". `search.js` no longer binds a Search button; `rivian-tires.js` drops the mobile Show Filters accordion, the switch-slider proxies and the tooltip rebuilders (the info buttons are in the markup), and its document `mouseenter`/`mouseleave` handlers ignore non-element targets, which threw when the pointer entered the window.
  - **On a phone** (600px and under) the pill row scrolls sideways behind a "Filters" pill with a count badge, and any pill, or that button, opens the whole set as a bottom sheet: a sticky header, one section per filter with the native select (the best picker on a phone), the sliders with presets, 3PMS and OEM as switch rows, and a sticky footer with Clear all and "Show N tires". The sheet is the same element as the row (`.rtg-filter-chips.is-sheet`), so there is one set of controls; it is a modal dialog for assistive tech, Escape and the backdrop close it, body scroll locks behind it, and leaving phone width closes it. The Help me choose button goes full width under the search field, as before.
  - `frontend/css/rivian-tires.css`: the mobile toggle, filter header, `.filter-container`/`.filter-group`, extended filters row, switch, slider wrapper, result count, mobile badge and active-filter chip blocks are gone; the filter bar, popover, option list, presets, sheet and results line take their place, on the same tokens (40px pills at 20px radius, 48px search field, 12px card radius, `color-mix()` accent tints). `.filter-wrapper` is no longer `overflow: hidden` because popovers hang below it. Range slider rules are scoped to `input.range-slider`. A `[hidden]` attribute inside the panel now wins over the display rules, which also lets the advisor button's own `hidden` (set when the advisor is not configured) take effect.
- README: the feature list describes the filter bar; the stale "type-ahead suggestions" line is gone.

### Tests
- `npm test`, `php -l` on the template and the contract checks pass; the bundle was exercised in a headless browser at 1280px and 390px: a list pick, a price preset, a toggle, a vehicle change that clears the size, Clear all, sort, the phone sheet (open, change a select, toggle OEM, Show N tires) and no horizontal overflow at 390px, with no page errors.
...
### Added
- **Tire Discovery judges load range per vehicle.** The R2 needs XL at minimum, and the queue had been offering SL tires in R2 sizes for review. `RTG_Tire_Qualifier::qualify()` now asks each platform a third question beside size and load index: is this tire built to at least your load range? A platform with a floor turns away a lighter construction, so an SL tire in an R2-only size is filed under Near misses with the reason "Load range SL is too light — R2 needs XL or higher" (code `load_range_low`); in a size both platforms take it still fits the R1 and is reported against the R1 alone. A listing that states no load range, or one in a spelling the qualifier cannot place, is surfaced with a `load_range_unknown` warning naming the floor to confirm, the same treatment an unlisted load index gets. Short on both load index and load range, a row carries both reasons. Rows already in the queue re-judge themselves on the next run, since machine statuses move with each sweep.
  - Sidewall order is `LOAD_RANGE_ORDER` (SL, XL, HL, C, D, E, F) with `RF` read as XL (`LOAD_RANGE_ALIASES`); `load_range_rank()` places a rating in it. Built-in floors are `VEHICLE_MIN_LOAD_RANGE` (R2 => XL; the R1 has none). `get_vehicle_min_load_ranges()` resolves the floor per wheels-table vehicle from the new `catalog_vehicle_min_load_range` setting: a saved rating wins, a saved `none` lifts the floor, blank means the built-in figure. The context key is `vehicle_min_load_range`.
  - Discovery settings, "What qualifies": the per-vehicle table gains a Load range column with a select per platform (Default, No minimum, or a rating "or higher"), saved by `save_catalog_settings_from_post()`.
...
- **The calculated efficiency score and grade.** The formula-derived 0-100 score and A-F grade left the cards in 1.51.0 and the admin form in 1.58.0 but were still computed on every save and import, exposed on the REST feed and a `POST /rtg/v1/efficiency` endpoint, offered as a sort key, averaged on the dashboard and printed on the share image. All of it is gone: `RTG_Database::calculate_efficiency()` and `recalculate_all_efficiency()`, the admin "Recalculate efficiency" action and its notice, the `rtg_calculate_efficiency` AJAX handler, the REST endpoint and feed fields, the `efficiency_score` sort key everywhere (the server-side fallback sort is now real-world efficiency), the `by_grade` and `avg_efficiency` dashboard aggregates, the `.rtg-grade` admin styles and the grade table in `CLAUDE.md`. Migration 27 drops `efficiency_score`, `efficiency_grade` and `idx_efficiency` from `rtg_tires` (`DB_VERSION` 27). Real-world efficiency from Rivian Roamer is unrelated and untouched.
  - `RTG_Database::insert_tire()` paired values with a positional format list that had to be kept in step with its defaults by hand; with two columns gone the list was not, and every format after them shifted by two (`bundle_link` was written through `%d` as "0", caught by `test_csv_carries_model_aliases_and_bundle_link`). Insert and update now both take their placeholders from one keyed `column_format()`, so a column can never be typed by its neighbour's slot again.
...
### Tests
- `tests/test-tire-qualifier.php`: an SL tire in an R2 size is rejected on load range with the reason naming the floor; XL, HL and E clear it; RF ranks as XL; an SL tire in a shared size fits the R1 only; an unlisted range warns rather than rejects; short on both counts gives both reasons; the floors resolve from settings with the built-in defaults.
...
Nothing visible to owners: the discovery rule and the efficiency removal are admin-side.

## [2.5.0] - 2026-09-12

### Changed
- **The admin panel was rebuilt on one design system.** Every Tire Guide screen now shares the same page header (title, one-line purpose, actions on the right), the same cards, tables, buttons, badges and form rows, and none of the fourteen views carries inline styling any more (about 400 `style=""` attributes are gone). The Roamer and Discovery pages had been written against WordPress's `form-table` / `wp-list-table` / `.button` markup and, in Roamer's case, a dark palette with hard-coded slate and blue hex values that never matched the light admin theme; both are on `.rtg-*` components now. `admin/css/admin-styles.css` gains the shared pieces the views needed: underline tabs (`.rtg-tabs`) for sections of one page, filter pills (`.rtg-pills`) for status filters, a toolbar, a key/value readout grid (`.rtg-kv-grid`) for sync status, stat-card tone variants, collapsible cards, styled `<details>`, an input group with a prefix addon, tag chips, radio/checkbox lists, a progress bar, small and quiet-danger button variants, table action clusters, and a sticky form footer (`.rtg-footer-actions.is-sticky`) with an "Unsaved changes" indicator. Notices the page scripts insert after an AJAX call (`.notice.inline`) now take the same look as the plugin's own. Row actions that only appeared on hover now also show on touch screens and on keyboard focus.
- **Menu.** Reordered to follow the work: Dashboard, Tires, Add New, Tire Discovery, Reviews, Wheels, Affiliate Links, Roamer Data, Analytics, Tools, Settings. "All Tires" is "Tires", "Stock Wheels" is "Wheels" (it has held third-party wheels since 2.4.0), "Roamer Sync" is "Roamer Data". "Share Image" and "Import / Export" are one **Tools** page in tabs (Import / Export, Share image, Data feed); the page keeps the `rtg-import` slug so the README link and old bookmarks work, and `rtg-share-image` still answers, opening Tools on that tab. New `parent_file` / `submenu_file` filters in `RTG_Admin` keep the right menu item lit on screens that have no entry of their own: editing a wheel highlights Wheels, editing an existing tire highlights Tires rather than Add New.
- **Dashboard** leads with a "Needs attention" list: a failed or stale discovery run (judged by `RTG_Health::evaluate()`), a failed Roamer sync, pending reviews, the discovery queue, broken links, missing links and missing images, each with a button to the screen that fixes it, and the items that are fine listed dimmed below so "all clear" is visible rather than assumed. The stat tiles link to their pages, the nested card-in-card stat blocks became key/value grids, and the JSON feed card moved to Tools.
- **Settings** is in tabs (General, Theme colors, Dropdown options, Affiliate domains, AI advisor, Analytics) with one form and a sticky Save, so a long page no longer means scrolling to the bottom to save. Theme colors get a native color picker beside each hex field, kept in step both ways; only the text field is named, so what saves is unchanged. The shortcode moved to the top of General with a Copy button.
- **Tire Discovery** is in tabs too: Review queue, Retailer coverage & prices, Settings. The queue tab opens by default; a settings save reopens the Settings tab. The status filters are pills with counts, the run status is a key/value grid with "Run discovery now" in the page header, the candidates' actions are small buttons, and the settings form is field rows with toggles instead of a `form-table`, grouped as Discovery, What qualifies, Prices and links, CJ Affiliate.
- **Roamer Data** puts the sync status and the two lists that need a person (ambiguous matches, unmatched tires) first; settings use toggles; linked, unlinked and hidden tires are collapsible cards driven by the shared `data-rtg-collapse` behavior (the per-ID handlers left `rtg-roamer.js`).
- **Tire form** gets a back link, a subtitle naming the tire, View page and Duplicate in the header, a sticky footer with Save and a quiet Delete, a proper input group for the image prefix, tag chips that reflect the field as it is typed, and the Roamer readout on the design system. **Reviews**, **Affiliate Links** and **Wheels** get pills, toolbars, action buttons and the shared header; the affiliate stat tiles filter the list on click; the link-check progress bar is a component. **Analytics** gets a segmented period control (its old `.rtg-filter-tab` class was never defined in the stylesheet, so the buttons rendered unstyled), light-theme chart colors in place of the dark-theme greys and white grid lines, and a message when the chart library fails to load instead of "Loading..." forever (H10 in the roadmap).
- Shared page behaviors in `admin-scripts.js`: `initTabs()` (`[data-rtg-tabs]`, the open tab in the URL hash as `#tab-name`, arrow-key movement, panels hidden not removed so a form spanning tabs submits whole), `initCollapsibles()`, `initColorFields()`, `initUnsavedIndicator()`, and a `[data-rtg-copy]` clipboard button that replaces the feed-URL-specific handler.

### Tests
- `npm test`, `php -l` on every touched file and the contract checks pass; the PHPUnit suite runs in CI (`test_menu_pages_registered` still finds `rtg-import`, which Tools keeps).

Nothing visible to owners: this release changes only the admin panel.
...
- **Size menu group headings were black on Windows.** A reader on Windows Chrome sent a screenshot of the size dropdown with its "18\" Wheels" headings in black on the dark popup. Chrome and Edge on Windows paint the native popup with the select's background but give `optgroup` labels the UA default color unless the page sets one; the plugin styled the select and never its options or groups, and macOS and Firefox happened to inherit. `rivian-tires.css` now paints `option` and `optgroup` in the guide's filter and sort selects with the input background and primary text color, and bolds the group label.
