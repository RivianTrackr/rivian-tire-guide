...
- **Filter Bar** — Search runs as you type. Below it, the vehicle switch and one pill per filter (Size, Brand, Category, Price, Warranty, 3PMS, OEM); a pill opens a popover with the options, or the slider with presets, and shows its value once set. On a phone the pill row scrolls sideways and any pill opens the full set as a bottom sheet with native pickers and a live "Show N tires" button.
- **Smart Search** — Word-prefix matching across brand, model, category and size, so "mich 275" finds Michelin tires in 275 sizes.
...
- **Filter Summary** — Set pills carry their value in accent, and the results line shows the count, the number of filters on, and Clear all.
...
- **Brand Policy** — By default the queue lists only the brands on the guide's own brand list: a listing from any other brand is judged and counted but never stored, and rows earlier runs kept for such brands are removed. The policy can be relaxed in Discovery settings to file uncovered brands under Near Misses or to surface them flagged.
