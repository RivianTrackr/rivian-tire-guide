...
- **Rate Limiting:** Transient-based rate limiter on review submissions (3 per 5-minute window per user; IP-based for guests). REST API rate limiting (60 reads/min, 10 writes/min). Public AJAX endpoints (catalog reads, click/search tracking) capped at 120–240 req/min per fingerprint.
