<?php
if ( ! defined( 'ABSPATH' ) ) {
    exit;
}
?>
<div id="rtg-user-reviews" class="rtg-user-reviews-page">
  <div class="rtg-user-reviews-header">
    <h2 id="rtg-user-reviews-title">Reviews</h2>
    <p id="rtg-user-reviews-count" class="rtg-user-reviews-subtitle"></p>
  </div>
  <div id="rtg-user-reviews-list" class="rtg-user-reviews-list">
    <div class="rtg-reviews-loading">Loading reviews...</div>
  </div>
  <div id="rtg-user-reviews-pagination" class="rtg-user-reviews-pagination"></div>
</div>
